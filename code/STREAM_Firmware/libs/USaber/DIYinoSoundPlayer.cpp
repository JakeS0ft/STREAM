...
#define MIN_SOUND_INTERVAL 100UL //Don't attempt to play sounds closer than 100 ms togeather
...
{
	while(millis() - mLastSoundTime < MIN_SOUND_INTERVAL)
	{
		delay(1); //Wait for enough time to pass before playing next sound
	}
	mLastSoundTime = millis();
...
bool DIYinoSoundPlayer::PlaySound(ESoundTypes aSoundType, uint16_t aIndex)
{
	...
	if(lSuccess)
	{
		if(eeHumSnd == aSoundType || eeLockupSnd == aSoundType)
		{
			//Send serial command to the DFPlayer so that hum or lockup sound loops
