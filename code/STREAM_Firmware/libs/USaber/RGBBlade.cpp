...
{
	int lRampStages = (int)((float)aRampTime / 50.0);
//	int lRampStages = MAX_RAMP_STAGES;
//	if(aRampTime < 1000)
//	{
//		lRampStages = MAX_RAMP_STAGES/2;
//	}
//	if(aRampTime >= 10000)
//	{
//		lRampStages = MAX_RAMP_STAGES*2;
//	}
	...
	int lDelay = int((float)aRampTime / (float)lRampStages);
...
void RGBBlade::RandomFlicker(int aLowerBound, int aUpperBound, unsigned long aUpdatePeriod)
{
...
void  RGBBlade::SineFlicker(float aAmplitude, unsigned long aUpdatePeroid)
{
	unsigned char lTempPowerLevels[3];
	...
	if(millis() - mLastFlickerUpdate > aUpdatePeroid)
	{
		...
		//Serial.print("Modifier=");Serial.println(lModifier);

		//Calculate new power levels
		for(int lChnl = 0; lChnl < 3; lChnl++)
		{
			float lTempPowerLevel = (float)mPowerLevels[lChnl] + (lModifier * (float)mPowerLevels[lChnl]);
			//float lTempPowerLevel = (float)mPowerLevels[lChnl] * lModifier;
			//Compensate for overshoot
			if(lTempPowerLevel > 255)
			{
				lTempPowerLevel = 255;
			}
			lTempPowerLevels[lChnl] = (unsigned char)lTempPowerLevel;
