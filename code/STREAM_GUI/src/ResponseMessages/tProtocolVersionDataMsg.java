 ...
 * @author Jake
 */
public class tProtocolVersionDataMsg extends ResponseMessageBase
{
    public tProtocolVersionDataMsg(ArrayList<Byte> aBytes)
    {
        mMsgId = (int) aBytes.get(0);
        mVersionMajor = aBytes.get(1);
        mVersionMinor = aBytes.get(2);
    }

    public tProtocolVersionDataMsg(Queue<Byte> aBytes)
    {
        mMsgId = aBytes.remove();
        mVersionMajor = aBytes.remove();
        mVersionMinor = aBytes.remove();
    }
